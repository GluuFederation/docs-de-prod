...
- `GLUU_LDAP_URL`: Address and port of LDAP server (default to `localhost:1636`).
- `GLUU_COUCHBASE_URL`: Address of Couchbase server (default to `localhost`).
- `GLUU_COUCHBASE_USER`: Username of Couchbase server (default to `admin`).
